...
ystart = c(Z.pool=0,P.pool=0,D.pool=2);# concentration as mols per liter
times = seq(0,24,1);#hourly

derivative = dNdt(times,ystart,params)
out = ode(y=ystart,times=times,func=dNdt,parms=params)

plot(out[,1],out[,4],type="l",xlab="time (hr)",ylab="Pool Concentration (mols/L)",ylim=c(0,2));#Phytoplankton Pool
